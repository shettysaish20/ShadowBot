...
function initializeNewSession() {
 // Don't generate session ID here - let backend create it
 currentSessionId = null;
 currentTranscription = '';
 conversationHistory = [];
 console.log('New conversation session initialized, waiting for backend session ID');
}
...
function setupGeminiIpcHandlers(geminiSessionRef) {
 ...
 try {
 console.log('=== IPC send-text-message handler called ===');
 console.log('Received text:', text);
 
 if (!text || typeof text !== 'string' || text.trim().length === 0) {
 console.log('Invalid text message, returning error');
